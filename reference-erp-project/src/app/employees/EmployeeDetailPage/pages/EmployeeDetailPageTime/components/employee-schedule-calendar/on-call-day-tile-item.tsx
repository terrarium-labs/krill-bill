...
interface OnCallDayTileItemProps {
  ...
  /**
   * `sm` matches compact weekly time-policy shift chips (employee schedule).
   * @default "default"
   */
  size?: "default" | "sm";
  /**
   * `type` = solid orange (same as year “OnCall” legend). `default` = group tag color.
   */
  calendarVariant?: YearCalendarCellVariant;
}

const sizeTriggerClasses: Record<NonNullable<OnCallDayTileItemProps["size"]>, string> = {
  default:
    "px-2 py-1 text-xs gap-1 min-h-8 [&_svg]:h-3.5 [&_svg]:w-3.5",
  sm: "px-1.5 py-0.5 text-[10px] leading-tight gap-0.5 min-h-8 [&_svg]:h-3 [&_svg]:w-3",
};
...
const OnCallDayTileItem = ({
  ...
  const baseClasses = cn(
    ...
    "font-medium truncate flex items-center min-w-0",
