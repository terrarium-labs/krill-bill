...
type OrderStatus = "completed" | "failed" | "in-progress" | "todo";
...
const STATUS_STYLES: Record<OrderStatus, { bg: string; border: string; text: string; label: string }> = {
...
const MOCK_SCHEDULES: TechnicianSchedule[] = [
    {
...
function Block({ block }: { block: ScheduleBlock }) {
    ...
    const st = STATUS_STYLES[block.status ?? "todo"];
...
export default function TechniciansTimelineWidget() {
    const { t } = useTranslation();
    ...
    const filtered = useMemo(() => {
        if (!searchQuery.trim()) return MOCK_SCHEDULES;
        const q = searchQuery.toLowerCase();
        return MOCK_SCHEDULES.filter((tech) => tech.name.toLowerCase().includes(q) || tech.role.toLowerCase().includes(q));
    }, [searchQuery]);
        ...
        <div className="flex flex-col h-full w-full overflow-hidden">
            ...
            <div className="shrink-0 flex items-center gap-3 px-3 py-2 border-b">
                <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm font-medium tabular-nums whitespace-nowrap min-w-[155px] text-center">
            ...
            <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-auto">
                <div style={{ width: EMPLOYEE_COL_W + GRID_W, minHeight: "100%" }}>
                    ...
                    {filtered.map((tech) => {
                        ...
                        return (
                            <div key={tech.id} className="flex border-b" style={{ height: ROW_H }}>
                                <div className="sticky left-0 z-10 shrink-0 bg-card border-r flex items-center gap-2 px-2.5" style={{ width: EMPLOYEE_COL_W }}>
            ...
            <div className="shrink-0 flex items-center justify-between px-3 py-1.5 border-t text-xs text-muted-foreground">
                <span>{t("common.showingOf", "Showing {{count}} of {{total}}", { count: filtered.length, total: MOCK_SCHEDULES.length })}</span>
                <div className="flex items-center gap-3">
                    {(Object.entries(STATUS_STYLES) as [OrderStatus, typeof STATUS_STYLES["todo"]][]).map(([key, s]) => (
