...
const EmployeePaymentMethodsCard: React.FC<EmployeePaymentMethodsCardProps> = () => {
    ...
    const loadPaymentMethods = async (pageToken: string | null = null, append = false) => {
        ...
        setIsLoading(true);
        try {
            const response = await getEmployeePaymentsMethods(orgId, "me", pageToken);
    ...
    const handleDeleteConfirm = async () => {
        if (!paymentMethodToDelete || !orgId) return;

        setIsDeleting(true);
        try {
            const response = await deleteEmployeePaymentsMethod(orgId, "me", paymentMethodToDelete.id);
            if (response.success) {
    ...
    const handleSetDefault = async (paymentMethod: EmployeePaymentMethod) => {
        ...
        try {
            const response = await postDefaultEmployeePaymentsMethod(orgId, "me", paymentMethod.id, {});
            if (response.success) {
