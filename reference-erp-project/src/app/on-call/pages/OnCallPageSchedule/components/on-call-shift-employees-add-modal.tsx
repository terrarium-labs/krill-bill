...
import { postOrgOnCallShiftEmployees } from "@/api/field-service/on-call/on-call-shifts";
import { Employee } from "@/types/employees/employees";

interface OnCallShiftEmployeesAddModalProps {
  open: boolean;
...
const OnCallShiftEmployeesAddModal = ({
  open,
  onOpenChange,
  shiftId,
  existingEmployeeIds,
  onEmployeesAdded,
}: OnCallShiftEmployeesAddModalProps) => {
  ...
  const fetchEmployees = async (query: string = "", pageToken: string | null = null) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          t("on-call.shifts.employees.errorLoading", "Failed to load employees")
        );
      }
    } catch (error) {
  ...
  const handleAddEmployee = async (employee: Employee) => {
    ...
    try {
      const response = await postOrgOnCallShiftEmployees(orgId, shiftId, {
        employees_ids: [employee.id],
      });
      if (response.error) {
        toast.error(
          t("on-call.shifts.employees.errorAdding", "Error adding employee")
        );
      } else {
        toast.success(
          t("on-call.shifts.employees.employeeAdded", "Employee added")
        );
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold">
            {t("on-call.shifts.employees.addEmployee", "Add employee")}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-4 flex-1 min-h-0">
          <SearchBar
            ...
            placeholder={t(
              "on-call.shifts.employees.searchPlaceholder",
              "Search employees..."
            )}
            className="w-full"
          />
              ...
              <div className="flex items-center justify-center py-12">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t(
                      "on-call.shifts.employees.loadingEmployees",
                      "Loading employees..."
                    )}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-center py-12">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <h3 className="text-lg font-medium">
                    {searchQuery
                      ? t(
                          "on-call.shifts.employees.noEmployeesFound",
                          "No employees found"
                        )
                      : t(
                          "on-call.shifts.employees.noEmployeesAvailable",
...
export default OnCallShiftEmployeesAddModal;
