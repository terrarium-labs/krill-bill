...
import type { OnCallHistoryTableColumnKey } from "./on-call-history-table";

export const CONFIGURABLE_COLUMNS: { id: OnCallHistoryTableColumnKey; label: string }[] = [
    { id: "id", label: "ID" },
    { id: "requirements", label: "Requirements" },
    { id: "resting_time_after_call", label: "Resting Time" },
    { id: "execution_error", label: "Execution Error" },
];

interface OnCallHistoryColumnSelectorProps {
...
export function OnCallHistoryColumnSelector({
    columnVisibility,
    columnOrder,
    onColumnVisibilityChange,
    onColumnOrderChange,
    onReset,
}: OnCallHistoryColumnSelectorProps) {
    ...
    const orderedColumns = (() => {
        ...
        const ordered = columnOrder.filter((id) =>
            configurableIds.includes(id as OnCallHistoryTableColumnKey),
        );
    ...
    const handleDrop = (targetId: string) => {
        ...
        const reordered = [...ids];
        const [moved] = reordered.splice(ids.indexOf(sourceId as OnCallHistoryTableColumnKey), 1);
        reordered.splice(ids.indexOf(targetId as OnCallHistoryTableColumnKey), 0, moved);
