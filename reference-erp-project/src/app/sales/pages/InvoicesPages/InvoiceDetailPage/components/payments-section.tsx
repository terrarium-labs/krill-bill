...
import { getSalesInvoicePayments, deleteSalesInvoicePayment } from "@/api/sales-invoices/payments/payments";
...
const PaymentsSection: React.FC<PaymentsSectionProps> = ({ invoiceId }) => {
    ...
    const { invoice, refreshInvoice } = useSaleInvoice();
    ...
    const fetchPayments = useCallback(async (pageToken: string | null = null, append = false) => {
        ...
        try {
            const response = await getSalesInvoicePayments(orgId, invoiceId, undefined, pageToken || undefined);
    ...
    const handleDeleteConfirm = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await deleteSalesInvoicePayment(orgId, invoiceId, paymentToDelete.id);
            if (response.success) {
