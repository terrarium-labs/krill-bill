...
interface HistoryClientLocationSectionProps {
    clientId: string;
    locationId: string;
    currentWorkOrderId?: string;
}

export function HistoryClientLocationSection({ clientId, locationId, currentWorkOrderId }: HistoryClientLocationSectionProps) {
    const { t } = useTranslation();
    const { orgId } = useParams<{ orgId: string }>();
    const [workOrders, setWorkOrders] = useState<WorkOrder[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    const defaultFilters = clientAndLocationFilterTemplate(clientId, locationId);
            ...
            <div className="flex flex-col gap-3 py-2">
                {workOrders.length === 0 ? (
                    <div className="text-sm text-muted-foreground py-4">
                        {t("workOrders.noWorkOrdersForLocation", "No work orders for this location")}
                    </div>
                ) : (
                    (() => {
