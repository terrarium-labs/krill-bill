...
import { postWorkOrderSupervisor } from '@/api/field-service/work-orders/supervisors/supervisors';
import { Employee } from '@/types/employees/employees';

interface WorkOrderSupervisorAddModalProps {
    open: boolean;
...
const WorkOrderSupervisorAddModal: React.FC<WorkOrderSupervisorAddModalProps> = ({
    open,
    onOpenChange,
    orgId,
    workOrderId,
    ...
    const fetchEmployees = async (query: string = "", pageToken: string | null = null) => {
        ...
        try {
            const response = await getOrgEmployees(orgId, undefined, undefined, undefined, undefined, undefined, workOrderId ||undefined, query || undefined, pageToken || undefined, undefined);
    ...
    const handleAddSupervisor = async (employee: Employee) => {
        if (!orgId || !workOrderId) return;

        setAddingEmployees(prev => new Set(prev).add(employee.id));
        try {
            const response = await postWorkOrderSupervisor(orgId, workOrderId, { supervisor_id: employee.id });
            if (response.success) {
                toast.success(t('workOrders.supervisorAddedSuccessfully', 'Supervisor added successfully'));
                setEmployees(prev => prev.filter(emp => emp.id !== employee.id));
                onSuccess?.();
            } else {
                toast.error(response.error || t('workOrders.errorAddingSupervisor', 'Error adding supervisor'));
            }
        } catch (error) {
            console.error('Error adding supervisor:', error);
            toast.error(t('workOrders.errorAddingSupervisor', 'Error adding supervisor'));
        } finally {
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col" showCloseButton={false}>
                <DialogHeader>
                    <DialogTitle className="text-lg font-semibold">
                        {t('workOrders.addSupervisor', 'Add Supervisor')}
                    </DialogTitle>
                </DialogHeader>
                                ...
                                {employees.map((employee) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <Button
                                                size="sm"
                                                onClick={() => handleAddSupervisor(employee)}
                                                disabled={isAdding}
                                                variant="outline"
                                                className="shrink-0"
                                            >
...
export default WorkOrderSupervisorAddModal;
