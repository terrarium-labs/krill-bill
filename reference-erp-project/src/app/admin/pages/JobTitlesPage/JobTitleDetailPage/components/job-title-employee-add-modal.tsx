...
import { postOrgJobTitleEmployee } from '@/api/orgs/job-titles/job-titles';
import { Employee } from '@/types/employees/employees';

interface AddEmployeeToJobTitleModalProps {
    open: boolean;
...
const AddEmployeeToJobTitleModal: React.FC<AddEmployeeToJobTitleModalProps> = ({
    open,
    onOpenChange,
    orgId,
    jobTitleId,
    ...
    const fetchEmployees = async (query: string = "", pageToken: string | null = null) => {
        ...
        try {
            const response = await getOrgEmployees(orgId, undefined, undefined, jobTitleId || undefined, undefined, undefined, undefined, query || undefined, pageToken || undefined, undefined);
            ...
            if (response.success) {
                ...
                toast.error(
                    t("jobTitles.employees.errorFetchingEmployees", "Failed to load employees")
                );
            }
        } catch (error) {
    ...
    const handleAddEmployee = async (employee: Employee) => {
        if (!orgId || !jobTitleId) {
            toast.error(t("jobTitles.employees.errorMissingIds", "Organization ID and Job Title ID are required"));
            return;
        }
        ...
        try {
            const response = await postOrgJobTitleEmployee(orgId, jobTitleId, {
                employees_ids: [employee.id]
            });

            if (response.success) {
                toast.success(
                    t("jobTitles.employees.employeeAddedSuccess", "Employee added successfully to job title")
                );
                ...
                toast.error(
                    response.error || t("jobTitles.employees.errorAddingEmployee", "Failed to add employee to job title")
                );
            }
        } catch (error) {
            console.error("Error adding employee to job title:", error);
            toast.error(
                t("jobTitles.employees.errorAddingEmployee", "Failed to add employee to job title")
            );
        } finally {
    ...
    if (!orgId || !jobTitleId) return null;
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col" showCloseButton={false}>
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
                        {t("jobTitles.employees.addEmployees", "Add Employees")}
                    </DialogTitle>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-4 flex-1 min-h-0">
                    ...
                    <SearchBar
                        ...
                        placeholder={t("jobTitles.employees.searchEmployees", "Search employees...")}
                        className="w-full"
                    />
                            ...
                            <div className="flex items-center justify-center py-12">
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        {t("jobTitles.employees.loadingEmployees", "Loading employees...")}
                                    </p>
                                </div>
                            </div>
                        ) : employees.length === 0 ? (
                            <div className="flex items-center justify-center py-12">
                                <div className="flex flex-col items-center gap-2">
                                    ...
                                    <h3 className="text-lg font-medium">
                                        {searchQuery
                                            ? t("jobTitles.employees.noEmployeesFound", "No employees found")
                                            : t("jobTitles.employees.noEmployees", "No employees available")
                                        }
                                    </h3>
                                    <p className="text-sm text-muted-foreground text-center">
                                        {searchQuery
                                            ? t(
                                                "jobTitles.employees.noEmployeesFoundDescription",
                                                "No employees found for '{{query}}'",
                                                { query: searchQuery }
                                            )
                                            : t(
                                                "jobTitles.employees.noEmployeesDescription",
                                                "No employees available to add to this job title"
                                            )
                                        }
                                    </p>
                                </div>
                                            ...
                                            >
                                                {isAdding ? (
                                                    <>
                                                        ...
                                                        {t("jobTitles.employees.adding", "Adding...")}
                                                    </>
                                                ) : (
                                                    <>
                                                        <Plus className="h-4 w-4" />
                                                        {t("jobTitles.employees.add", "Add")}
                                                    </>
                                                )}
                                            </Button>
                                        </div>
...
export default AddEmployeeToJobTitleModal;
