...
const SoftSkillsPage = () => {
    ...
    const fetchSkills = async (query: string = "", page_token?: string) => {
        ...
        try {
            const response = await getOrgSkills(orgId, "soft", query, page_token || undefined);
            if (response.success) {
                if (page_token) {
                    setSkills((prev) => [
                        ...prev,
                ...
                toast.error(t("skills.softSkills.errorFetching", "Error fetching soft skills"));
            }
        } catch (error) {
            toast.error(t("skills.softSkills.errorFetching", "Error fetching soft skills"));
        } finally {
    ...
    const handleDeleteSkill = async () => {
        ...
        try {
            ...
            if (response.success) {
                toast.success(t("skills.softSkills.deleted", "Soft skill deleted successfully"));
                setSkills(prev => prev.filter(s => s.id !== skillToDelete.id));
            } else {
                toast.error(t("skills.softSkills.errorDeleting", "Error deleting soft skill"));
            }
        } catch (error) {
            toast.error(t("skills.softSkills.errorDeleting", "Error deleting soft skill"));
        } finally {
        ...
        <>
            <div className="flex items-center gap-2">
                <SearchBar
                    ...
                    placeholder={t("skills.softSkills.searchPlaceholder", "Search soft skills...")}
                />
                ...
                <Button onClick={() => {
                    ...
                    {t("skills.softSkills.addSkill", "New soft skill")}
                </Button>
            </div>
