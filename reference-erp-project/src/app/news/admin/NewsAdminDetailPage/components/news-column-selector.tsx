...
import type { NewsTableColumnKey } from "./news-table";

export const CONFIGURABLE_COLUMNS: { id: NewsTableColumnKey; label: string }[] = [
    { id: "id", label: "ID" },
...
interface NewsColumnSelectorProps {
...
export function NewsColumnSelector({
    columnVisibility,
    columnOrder,
    onColumnVisibilityChange,
    onColumnOrderChange,
    onReset,
}: NewsColumnSelectorProps) {
    ...
    const orderedColumns = (() => {
        ...
        const ordered = columnOrder.filter((id) =>
            configurableIds.includes(id as NewsTableColumnKey),
        );
    ...
    const handleDrop = (targetId: string) => {
        ...
        const reordered = [...ids];
        const [moved] = reordered.splice(ids.indexOf(sourceId as NewsTableColumnKey), 1);
        reordered.splice(ids.indexOf(targetId as NewsTableColumnKey), 0, moved);
