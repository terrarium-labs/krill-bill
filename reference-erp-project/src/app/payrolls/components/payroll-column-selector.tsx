...
import type { PayrollTableColumnKey } from "./payrolls-table";

export const CONFIGURABLE_COLUMNS: { id: PayrollTableColumnKey; label: string }[] = [
    { id: "id", label: "ID" },
    { id: "employee", label: "Employee" },
    { id: "period", label: "Period" },
    { id: "payment_date", label: "Payment Date" },
    { id: "earnings_total", label: "Earnings" },
    { id: "deductions_total", label: "Deductions" },
    { id: "net_amount_to_receive", label: "Net Amount" },
    { id: "company_costs_total", label: "Company Contrib." },
    { id: "gross_payroll_amount", label: "Company Cost" },
];

interface PayrollColumnSelectorProps {
...
export function PayrollColumnSelector({
    columnVisibility,
    columnOrder,
    onColumnVisibilityChange,
    onColumnOrderChange,
    onReset,
}: PayrollColumnSelectorProps) {
    ...
    const orderedColumns = (() => {
        ...
        const ordered = columnOrder.filter((id) =>
            configurableIds.includes(id as PayrollTableColumnKey),
        );
    ...
    const handleDrop = (targetId: string) => {
        ...
        const reordered = [...ids];
        const [moved] = reordered.splice(ids.indexOf(sourceId as PayrollTableColumnKey), 1);
        reordered.splice(ids.indexOf(targetId as PayrollTableColumnKey), 0, moved);
    ...
    return (
        <Popover>
            <Tooltip>
                ...
                <TooltipContent>
                    {t("table.columnSelectorTooltip", "Show, hide and reorder columns")}
                </TooltipContent>
            </Tooltip>
