...
import type { ClientTableColumnKey } from "./clients-table";

export const CONFIGURABLE_COLUMNS: { id: ClientTableColumnKey; label: string }[] = [
    { id: "id", label: "ID" },
...
export function ClientColumnSelector({
    columnVisibility,
    columnOrder,
    onColumnVisibilityChange,
    onColumnOrderChange,
    onReset,
}: ClientColumnSelectorProps) {
    ...
    const orderedColumns = (() => {
        ...
        const ordered = columnOrder.filter((id) =>
            configurableIds.includes(id as ClientTableColumnKey),
        );
    ...
    const handleDrop = (targetId: string) => {
        ...
        const reordered = [...ids];
        const [moved] = reordered.splice(ids.indexOf(sourceId as ClientTableColumnKey), 1);
        reordered.splice(ids.indexOf(targetId as ClientTableColumnKey), 0, moved);
