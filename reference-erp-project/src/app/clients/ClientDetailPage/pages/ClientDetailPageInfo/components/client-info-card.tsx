...
export const ClientInfoCard: React.FC<ClientInfoCardProps> = ({ onEdit }) => {
    ...
    return (
        <Card className="w-full shadow-none">
            ...
            <CardContent className="space-y-4">
                ...
                <IconInfoItem
                    icon={"globe"}
                    label={t('clients.website', 'Website')}
                    value={client.url}
                    link
                    linkValue={client.url}
                    onEmptyClick={onEdit}
                />
                        ...
                        <div className="pt-2">
                            <div className="flex items-center justify-between gap-2">
                                <p className="text-xs text-muted-foreground font-semibold">
                                    {t('clients.customFields', 'Custom Basic Fields')}
                                </p>
                ...
                {(client.risk !== null && client.risk !== undefined) ||
                    ...
                    <div className="pt-4 border-t border-border space-y-4">
                        <p className="text-xs text-muted-foreground font-semibold mb-3">
                            {t('clients.financialInformation', 'Financial Information')}
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {client.risk !== null && client.risk !== undefined && (
                                <IconInfoItem
                ...
                {client.sections && client.sections.length > 0 && client.sections.filter((field) => field.handler === 'financials').length > 0
                    && getFinancialFields()
                    ...
                    && (
                        <div className="pt-2">
                            <div className="flex items-center justify-between gap-2">
                                <p className="text-xs text-muted-foreground font-semibold">
                                    {t('clients.customFinancialFields', 'Custom Financial Fields')}
                                </p>
                                <Button variant="ghost" size="icon" onClick={() => navigate(`/${orgId}/admin/fields/clients`)}>
                                    <Settings className="h-4 w-4 text-muted-foreground" />
                                </Button>
                ...
                {client.sections
                    && client.sections.length > 0
                    && client.sections.filter((field) => field.handler !== 'basic' && field.handler !== 'financials').length > 0
                    && client.sections.filter((field) => field.handler !== 'basic' && field.handler !== 'financials').map((section) => {
