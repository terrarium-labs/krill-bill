...
import { getOrgAdminTicket } from "@/api/field-service/tickets/tickets";
...
interface TicketViewModalProps {
  ...
  /** When this value changes, the modal refetches the ticket (e.g. after close/reopen). */
...
const TicketViewModal: React.FC<TicketViewModalProps> = ({
  ...
  const fetchTicket = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await getOrgAdminTicket(orgId, ticketId);
      if (response.success) {
  ...
  // Refetch when parent signals an update (e.g. after close/reopen)
  useEffect(() => {
    if (open && ticketId && refreshTrigger != null && refreshTrigger > 0) {
      fetchTicket();
    }
  }, [refreshTrigger]);
