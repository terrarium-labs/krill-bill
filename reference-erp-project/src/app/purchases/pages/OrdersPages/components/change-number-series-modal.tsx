    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[500px]" showCloseButton={false}>
                <DialogHeader>
                    <DialogTitle>{t('orders.changeNumberSeriesTitle', 'Change Number Series')}</DialogTitle>
                </DialogHeader>

                <div className="space-y-4 py-4">
                    <div className="space-y-2">
                        <Label htmlFor="serial-number">
                            {t('orders.numberSeries', 'Number Series')}
                        </Label>
                        <MultiSelectApi
                            fetchOptions={getOrgSerialNumbers}
                            fetchArgs={[orgId, "orders"]}
                            ...
                            placeholder={t('orders.selectNumberSeries', 'Select number series...')}
                            searchPlaceholder={t('orders.searchNumberSeries', 'Search number series...')}
                            emptyText={t('orders.noNumberSeriesFound', 'No number series found for orders')}
                            disabled={isLoading}
                            maxCount={1}
                            className="w-full"
                        />
