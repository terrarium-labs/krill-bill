...
export type ItemRateTableColumnKey =
    | "id"
    | "name"
    | "status"
    | "valid_from"
...
type ItemRatesTableProps = {
    rates: Rate[];
    isLoading: boolean;
    searchQuery: string;
    onViewRate: (rateId: string) => void;
    onAddRate?: () => void;
    renderActions?: (rate: Rate) => ReactNode;
    /** Structural column hiding (for embedding). Not saved to preferences. */
    hiddenColumns?: ItemRateTableColumnKey[] | ItemRateTableColumnKey;
    /** TanStack column visibility (from useItemRatesTablePreferences) */
...
const ItemRatesTableComponent = ({
    ...
    const { wrapRowWithContextMenu } = useTableContextMenu<Rate>(renderActions);
    ...
    const columns = useMemo<ColumnDef<Rate>[]>(() => {
        const cols: ColumnDef<Rate>[] = [
            ...
            },
            {
                ...
                header: t("rates.validFrom", "Valid From"),
                enableResizing: true,
                size: 120,
                cell: ({ row }) => {
            ...
            },
            {
                ...
                header: t("rates.validTo", "Valid To"),
                enableResizing: true,
                size: 120,
                cell: ({ row }) => {
    ...
    return (
        <div className="w-full overflow-x-auto">
            ...
            >
                                ...
                                <div className="flex items-center justify-center space-y-4 flex-col">
                                    ...
                                    <div className="flex flex-col items-center justify-center">
                                        <h3 className="text-lg font-medium">
                                            {searchQuery
                                                ? t("rates.noResultsFound", "No rates found")
                                                : t("rates.noRatesTitle", "No rates yet")}
                                        </h3>
                                        <p className="text-muted-foreground">
                                            {searchQuery
                                                ? t("rates.noResultsDescription", "No rates match your search for '{{searchQuery}}'", { searchQuery })
                                                : t("rates.noRatesDescription", "Start by adding your first rate")}
                                        </p>
                                    </div>
                                    {onAddRate && (
                ...
                >
                    {({ row }) => {
                        const rate = row.original as Rate;
                        return wrapRowWithContextMenu(
                            rate,
                            <TableRowRaw
                                key={row.id}
                                className="hover:bg-muted/50 cursor-pointer"
                                onClick={() => onViewRate(rate.id)}
...
export const ItemRatesTable = memo(ItemRatesTableComponent);
export default ItemRatesTable;
