...
import type { ApiKeyTableColumnKey } from "./api-keys-table";

export const CONFIGURABLE_COLUMNS: { id: ApiKeyTableColumnKey; label: string }[] = [
    { id: "id", label: "ID" },
    { id: "name", label: "Name" },
    { id: "display_key", label: "API Key" },
    { id: "created_at", label: "Created" },
];

interface ApiKeyColumnSelectorProps {
...
export function ApiKeyColumnSelector({
    columnVisibility,
    columnOrder,
    onColumnVisibilityChange,
    onColumnOrderChange,
    onReset,
}: ApiKeyColumnSelectorProps) {
    ...
    const orderedColumns = (() => {
        ...
        const ordered = columnOrder.filter((id) =>
            configurableIds.includes(id as ApiKeyTableColumnKey),
        );
    ...
    const handleDrop = (targetId: string) => {
        ...
        const reordered = [...ids];
        const [moved] = reordered.splice(ids.indexOf(sourceId as ApiKeyTableColumnKey), 1);
        reordered.splice(ids.indexOf(targetId as ApiKeyTableColumnKey), 0, moved);
