...
export function MultiSelectApi({
  ...
  const loadOptions = React.useCallback(async (
    ...
    try {
      const response = await fetchOptions(...fetchArgs, query, pageToken)
  ...
  }, [fetchOptions, fetchArgs, optionsKey, nextPageToken, customValueKey, customLabelKey, itemsMap, paramsEnabled])
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} modal={true}>
      ...
      {/* DROPDOWN CONTENT */}
      <PopoverContent 
        className={cn("p-0 w-auto min-w-[var(--radix-popover-trigger-width)] z-[10000] pointer-events-auto backdrop-blur-sm", popoverClassName)}
        side="bottom"
        sideOffset={8}
