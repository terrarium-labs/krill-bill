...
/**
 * Get primary accent color classes
 * Uses the dynamic accent color stored in CSS variable
 */
export const getAccentColorClasses = (): string => {
  return 'text-[color:var(--accent-color)] hover:text-[color:var(--accent-700)] dark:hover:text-[color:var(--accent-400)]';
};
...
export const getAccentBgClasses = (): string => {
  return 'bg-[color:var(--accent-color)] hover:bg-[color:var(--accent-700)] dark:bg-[color:var(--accent-color)] dark:hover:bg-[color:var(--accent-400)] text-[color:var(--accent-foreground)]';
};

/**
 * Get accent color classes for borders
 * Uses the dynamic accent color stored in CSS variable
 */
export const getAccentBorderClasses = (): string => {
  return 'border-[color:var(--accent-border)]';
};

/**
 * Get accent color ring/focus classes
 * Uses the dynamic accent color stored in CSS variable
 */
export const getAccentRingClasses = (): string => {
  return 'focus:ring-[color:var(--accent-color)] dark:focus:ring-[color:var(--accent-500)]';
};

/**
 * Get accent background with lighter shade (for backgrounds/badges)
 * Uses the dynamic accent color stored in CSS variable
 */
export const getAccentBgLightClasses = (): string => {
  return 'bg-[color:var(--accent-background)]';
};

/**
 * Get accent color for text with light background
 * Uses the dynamic accent color stored in CSS variable
 */
export const getAccentTextLightBgClasses = (): string => {
  return 'text-[color:var(--accent-color)]';
};

/**
 * Get soft accent background classes (muted/subtle)
 * Uses lighter shades for a softer appearance
 * Perfect for inactive or secondary elements
 */
export const getAccentBgSoftClasses = (): string => {
  return 'bg-[color:var(--accent-background)] hover:bg-[color:var(--accent-200)] dark:hover:bg-[color:var(--accent-800)]';
};

/**
 * Get soft accent text classes (for light backgrounds)
 * Uses medium shades for text on soft backgrounds
 */
export const getAccentTextSoftClasses = (): string => {
  return 'text-[color:var(--accent-color)] dark:text-[color:var(--accent-300)]';
};

/**
 * Get soft accent border classes
 * Uses softer shades for subtle borders
 */
export const getAccentBorderSoftClasses = (): string => {
  return 'border-[color:var(--accent-border)]';
};
